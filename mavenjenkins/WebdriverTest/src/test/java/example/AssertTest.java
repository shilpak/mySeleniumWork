...
public class AssertTest {
	...
	public void assignmentOperatorsExplored(){ 
		...
		num *= 2; assertEquals("*= multiplies by", 16, num);
		num /= 4; assertEquals("/= divides by", 4, num);
		num %=3; assertEquals("%= modulus of", 1, num);
	}


	@Test public void incrementDecrementOperatorsExplored(){ 
		int num = 10; 
		assertEquals(11, ++num); 
		assertEquals(10, --num); 
		assertEquals(10, num++); 
	...
	@Test public void booleanOperatorsExplored(){ 
		assertTrue( 4 == 4 ); 
		assertTrue(4 != 5); 
		assertTrue(3 < 4); 
		assertTrue(5 > 4); 
